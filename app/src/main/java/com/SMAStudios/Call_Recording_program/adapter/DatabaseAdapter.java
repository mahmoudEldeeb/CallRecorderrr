package com.SMAStudios.Call_Recording_program.adapter;
...
import com.SMAStudios.Call_Recording_program.model.PriorityContactModel;
...
//		db.close();
	...
	public long updateName( int id, String name) {
		SQLiteDatabase db = this.getWritableDatabase();
		ContentValues values = new ContentValues();
		values.put(COLUMN_PHONENUMBER, name);
		long index = db.update(TABLE_INBOX, values, COLUMN_ID+"="+id, null);
//		db.close();
		return index;
	}
